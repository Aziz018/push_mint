...
if [ $num_args -le 3 ]; then
    rule=3
elif [ $num_args -le 5 ]; then
    rule=13
elif [ $num_args -le 100 ]; then
    rule=700
elif [ $num_args -le 500 ]; then
    rule=5500
fi
...
if [ -x "$push_swap_path" ]; then
    while [[ $I -ne $N ]]; do
        ...
        sequence=$(seq $min $max | shuf -n $num_args)
        
        # Add code to generate unique sequences if desired
        
        $push_swap_path $sequence > .out
        cat .out | ./checker_linux $sequence 
        moves=$(cat .out | wc -l)
        rm -rf .out
